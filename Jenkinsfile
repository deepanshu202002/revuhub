  ...
  steps {
    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
      sh '''
        ...
        # Check if security groups exist and import them if not already in state
        if ! terraform state list | grep -q aws_security_group.alb_sg; then
          terraform import aws_security_group.alb_sg sg-035dc75746aa00f1a || true
        fi

        if ! terraform state list | grep -q aws_security_group.backend_sg; then
          terraform import aws_security_group.backend_sg sg-03fd1636f5e9bdf75 || true
        fi
